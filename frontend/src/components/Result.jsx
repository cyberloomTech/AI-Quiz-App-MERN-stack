...
const Result = ({ score }) => {
  const { state, handleRetryBtn, handleCheckBtn } = useContext(MyContext);
  ...
  const sendResult = async () => {
    try {
      ...
      const res = await axios.post("/api/history", historyObject)
      ...
      {!state.isCheck && (
        <div className="w-full max-w-4xl mx-auto mt-8 bg-white p-6">
          <h1 className="text-4xl font-bold text-center mb-4 text-slate-700">
            Score: {score / state.filteredQuestion.length * 100}%
          </h1>
